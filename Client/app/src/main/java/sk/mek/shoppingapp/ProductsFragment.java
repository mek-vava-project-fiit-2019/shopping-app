...
import sk.mek.shoppingapp.controller.modelmanager.ProductManager;
...
public class ProductsFragment extends Fragment implements IDynamicGrid, BottomNavigationView.OnNavigationItemSelectedListener, IAsyncResult<Product[]> {
    ...
    public void onStart() {
        ...
        String url = BuildConfig.CONFIG_PRODUCT_BY_CATEGORY_API_PATH.replace("{category}", "food");

        productManager.getByArgs(this, BuildConfig.CONFIG_SERVER_PATH + "/" + url);
    }
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
        ...
        switch (menuItem.getItemId()) {
            case R.id.nav_food:
                // nacitanie potravin
                category = BuildConfig.CONFIG_CATEGORY_FOOD;

                break;
            case R.id.nav_clothing:
                //nacitanie oblecenia

                category = BuildConfig.CONFIG_CATEGORY_CLOTHES;
                break;
            case R.id.nav_electronics:
                //nacitanie ostatnych produktov
                category = BuildConfig.CONFIG_CATEGORY_ELECTRONICS;
                break;
        }


        System.out.println("SELECTING products of category: " + category);
        String url = BuildConfig.CONFIG_PRODUCT_BY_CATEGORY_API_PATH.replace("{category}",category);
